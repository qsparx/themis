...
public class ThemisModule extends ReactContextBaseJavaModule {
  ...
  /* MARK: Secure Session */
  @ReactMethod
  public void secureSessionCreate(ReadableArray sessionId,
                                ReadableArray privateKey,
                                Callback successCallback,
                                Callback errorCallback)
  {
    ...
    try {
      byte[] privateKeyBinary = dataDeserialize(privateKey);
      PrivateKey keyPrivateKey = new PrivateKey(privateKeyBinary);
      byte[] sessionIdBinary = dataDeserialize(sessionId);
      SecureSession secureSession = new SecureSession(sessionIdBinary, keyPrivateKey);
      WritableArray response = dataSerialize(secureSession.toByteArray());
      successCallback.invoke(response);
    } catch (Exception e) {
      WritableMap error = new WritableNativeMap();
      error.putString("message", e.toString());
  ...
  @ReactMethod
  public void secureSessionConnect(ReadableArray session,
                                 Callback successCallback,
                                 Callback errorCallback)
  {
    try {
      byte[] sessionBinary = dataDeserialize(session);
      SecureSession secureSession = new SecureSession(sessionBinary);
      byte[] request = secureSession.connect();
      WritableArray response = dataSerialize(request);
      successCallback.invoke(response);
    } catch (Exception e) {
      WritableMap error = new WritableNativeMap();
      error.putString("message", e.toString());
  ...
  @ReactMethod
  public void secureSessionWrap(ReadableArray session,
                              String message,
                              Callback successCallback,
                              Callback errorCallback)
  {
    try {
      ...
      SecureSession secureSession = new SecureSession(sessionBinary);
      byte[] messageBinary = message.getBytes(StandardCharsets.UTF_8);
      byte[] wrapped = secureSession.wrap(messageBinary);
      WritableArray response = dataSerialize(wrapped);
      successCallback.invoke(response);
    } catch (Exception e) {
      WritableMap error = new WritableNativeMap();
      error.putString("message", e.toString());
  ...
  @ReactMethod
  public void secureSessionUnwrap(ReadableArray session,
                                ...
                                Callback errorCallback)
  {
    try {
      ...
      SecureSession secureSession = new SecureSession(sessionBinary);
      byte[] messageBinary = dataDeserialize(message);
      byte[] unwrapped = secureSession.unwrap(messageBinary);
      WritableArray response = dataSerialize(unwrapped);
      successCallback.invoke(response);
    } catch (Exception e) {
      WritableMap error = new WritableNativeMap();
      error.putString("message", e.toString());
  ...
  @ReactMethod
  public void secureSessionIsEstablished(ReadableArray session,
                                       Callback successCallback,
                                       Callback errorCallback)
  {
    try {
      byte[] sessionBinary = dataDeserialize(session);
      SecureSession secureSession = new SecureSession(sessionBinary);
      boolean isEstablished = secureSession.isEstablished();
      successCallback.invoke(isEstablished);
    } catch (Exception e) {
      WritableMap error = new WritableNativeMap();
      error.putString("message", e.toString());
      errorCallback.invoke(error);
