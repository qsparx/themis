...
// Secure Session
export function secureSessionCreate64(
  sessionId64: String,
  privateKey64: String): Promise<string> {

  checkInput(sessionId64, "sessionId64");
  const sessionId = convertInputBase64(sessionId64, "sessionId64");
  const privateKey = convertInputBase64(privateKey64, "privateKey64");

  return new Promise((resolve, reject) => {
    Themis.secureSessionCreate(sessionId, privateKey, (session: any) => {
      resolve(Buffer.from(new Uint8Array(session)).toString("base64"))
    }, (error: any) => {
      reject(error)
    })
  })
...
export function secureSessionConnect64(
  session64: String): Promise<string> {

  const session = convertInputBase64(session64, "session64");

  return new Promise((resolve, reject) => {
    Themis.secureSessionConnect(session, (request: any) => {
      resolve(Buffer.from(new Uint8Array(request)).toString("base64"))
    }, (error: any) => {
      reject(error)
    })
  })
...
export function secureSessionWrap64(
  session64: String,
  ...
  return new Promise((resolve, reject) => {
    Themis.secureSessionWrap(session, message, (wrapped: any) => {
      resolve(Buffer.from(new Uint8Array(wrapped)).toString("base64"))
    }, (error: any) => {
      reject(error)
    })
  })
...
export function secureSessionUnwrap64(
  session64: String,
  message64: String): Promise<string> {

  const session = convertInputBase64(session64, "session64");
  const message = convertInputBase64(message64, "message64");

  return new Promise((resolve, reject) => {
    Themis.secureSessionUnwrap(session, message, (unwrapped: any) => {
      resolve(Buffer.from(new Uint8Array(unwrapped)).toString())
    }, (error: any) => {
      reject(error)
    })
  })
...
export function secureSessionIsEstablished64(
  session64: String): Promise<boolean> {

  const session = convertInputBase64(session64, "session64");

  return new Promise((resolve, reject) => {
    Themis.secureSessionIsEstablished(session, (isEstablished: boolean) => {
      resolve(isEstablished)
    }, (error: any) => {
      reject(error)
    })
  })
