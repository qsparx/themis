...
// Secure Session
export function secureSessionCreate64(sessionId64, privateKey64) {
  checkInput(sessionId64, "sessionId64");
  const sessionId = convertInputBase64(sessionId64, "sessionId64");
  const privateKey = convertInputBase64(privateKey64, "privateKey64");

  return new Promise((resolve, reject) => {
    Themis.secureSessionCreate(sessionId, privateKey, (session) => {
...
export function secureSessionConnect64(session64) {
  const session = convertInputBase64(session64, "session64");

  return new Promise((resolve, reject) => {
    Themis.secureSessionConnect(session, (request) => {
...
export function secureSessionWrap64(session64, message) {
  checkInput(message, "message");
  const session = convertInputBase64(session64, "session64");

  return new Promise((resolve, reject) => {
    Themis.secureSessionWrap(session, message, (wrapped) => {
...
export function secureSessionUnwrap64(session64, message64) {
  const session = convertInputBase64(session64, "session64");
  const message = convertInputBase64(message64, "message64");

  return new Promise((resolve, reject) => {
    Themis.secureSessionUnwrap(session, message, (unwrapped) => {
...
export function secureSessionIsEstablished64(session64) {
  const session = convertInputBase64(session64, "session64");

  return new Promise((resolve, reject) => {
    Themis.secureSessionIsEstablished(session, (isEstablished) => {
