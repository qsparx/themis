...
#import "RCTThemis.h"
#import <themis/ssession.h>
#import <themis/ssession_transport_interface.h>
...
/* MARK: Secure Session */
RCT_EXPORT_METHOD(secureSessionCreate:(NSArray*) sessionId
                  privateKey:(NSArray*) privateKey
                  successCallback:(RCTResponseSenderBlock) successCallback
                  errorCallback:(RCTResponseErrorBlock) errorCallback)
{
  ...
  sessionIdBinary = [RCTThemis dataDeserialize:sessionId error:&error];
  if (sessionIdBinary == nil) {
    errorCallback(error);
    return;
  }

  // Create a transport interface for the session
  TSSessionTransportInterface *transport = [[TSSessionTransportInterface alloc] init];
  
  TSSession *secureSession = [[TSSession alloc] initWithUserId:sessionIdBinary
  ...
  // Store the session and transport interface
  self.currentSession = secureSession;
  self.currentTransport = transport;

  NSData *request = [secureSession connectRequest:&error];
  if (request == nil) {
    errorCallback(error);
    return;
  }

  NSArray *response = [RCTThemis dataSerialize:request];
  successCallback(@[response]);
}

RCT_EXPORT_METHOD(secureSessionConnect:(NSArray*) session
                  successCallback:(RCTResponseSenderBlock) successCallback
                  errorCallback:(RCTResponseErrorBlock) errorCallback)
{
  if (self.currentSession == nil) {
    errorCallback(SCERROR(-1, @"No active session"));
    return;
  }

  NSError *error;
  NSData *request = [self.currentSession connectRequest:&error];
  if (request == nil) {
    errorCallback(error);
    return;
  }
...
RCT_EXPORT_METHOD(secureSessionWrap:(NSArray*) session
                  ...
                  errorCallback:(RCTResponseErrorBlock) errorCallback)
{
  ...
  NSData *wrapped = [self.currentSession wrapData:messageBinary error:&error];
  if (wrapped == nil) {
    errorCallback(error);
    return;
  }

  NSArray *response = [RCTThemis dataSerialize:wrapped];
  successCallback(@[response]);
}

RCT_EXPORT_METHOD(secureSessionUnwrap:(NSArray*) session
                  ...
                  errorCallback:(RCTResponseErrorBlock) errorCallback)
{
  ...
  NSData *unwrapped = [self.currentSession unwrapData:messageBinary error:&error];
  if (unwrapped == nil) {
    errorCallback(error);
    return;
  }

  NSArray *response = [RCTThemis dataSerialize:unwrapped];
  successCallback(@[response]);
}

RCT_EXPORT_METHOD(secureSessionIsEstablished:(NSArray*) session
                  successCallback:(RCTResponseSenderBlock) successCallback
                  errorCallback:(RCTResponseErrorBlock) errorCallback)
{
  if (self.currentSession == nil) {
  ...
  BOOL isEstablished = [self.currentSession isSessionEstablished];
  successCallback(@[@(isEstablished)]);
}
